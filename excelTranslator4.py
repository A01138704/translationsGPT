...
def translate_text(text):
    try:
        response = client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": f"Translate the following text to English, when you finish translating use the stop string ///: {text}"}],
        )
        translated_text = response['choices'][0]['message']['content'].strip()  # Adjust how you access the translated text based on the API's response structure
