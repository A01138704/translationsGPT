...
def translate_text(text):
    ...
    for key, value in translation_memory.items():
        if levenshtein_distance(text, key) <= 3:  # Adjust the threshold as needed
            print(red("Translation found in memory."))
            return value
    ...
    try:
        ...
        translation_memory[text].append(translated_text)
        print(yellow("Translation added to memory."))
...
output_file_path = "translated_data_memory_6.csv"  # The output CSV file path where the translations will be saved
debug_file_path = "translation_debug_6.txt"  # The debug file path to store debug information
