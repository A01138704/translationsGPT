...
def translate_text_chunks_and_save(chunks, output_file_path):
    for index, chunk in enumerate(chunks):
        # Prepare the translation request for each chunk independently
        messages = [
            {"role": "system", "content": "You are a translation assistant. You always translate the provided text to English only."},
        ...
        try:
            completion = client.chat.completions.create(
                ...
                messages=messages,
                temperature=0.7,
                stream=False  # Ensure that stream is turned off
            )
            translated_chunk = completion.choices[0].text.strip()
        except Exception as e:
            print(f"Error during translation of chunk {index + 1}: {e}")
            continue

        # Print original and translated text with color coding
        print(f"Chunk {index + 1}/{len(chunks)} - Original Text:")
        print(blue(chunk))
        print(f"Chunk {index + 1}/{len(chunks)} - Translated Text:")
